...
rm(list = ls())
enzym <- "optimum_LBamidase" 
yyyymmdd <- "20230526"
pH_row <- "pH95_" # new entry, change for each pH

# Read in the plate template
folder_path <- file.path(paste0("data/", yyyymmdd, "/"))
file_path <- list.files(folder_path, pattern = "20230526_Template_pH_optimum_LBamidase_4NPbutyrate_410nm_347nm", full.names = T)
...
dat_normalized <- dat4 %>%
  dplyr::mutate(mean_1 = cur_data()[[5]]-cur_data()[[2]]) %>%
  dplyr::mutate(mean_2 = cur_data()[[6]]-cur_data()[[3]]) %>%
  dplyr::mutate(mean_3 = cur_data()[[7]]-cur_data()[[4]]) %>%
