...
enzym <- "p55" 
yyyymmdd <- "20230619"
pH_row <- "45°C" # new entry, change for each temperature
...
tmafils <- list.files(paste0(folder_path, "/"), pattern = paste0(pH_row, enzym), full.names = T) 
...
# Select the temperature of interest
ind <- which(grepl(pH_row, newnam))
ran <- (ind[1]-12):(ind[1]-7)
fullran <- c(1, ran, ind, ind+3)
newnam2 <- newnam[fullran]

# Clean up the data
tma1 <- tma %>%
  dplyr::select(fullran) %>%
  dplyr::mutate(Time = round(as.numeric(lubridate::hms(stringr::word(Time, sep = " ", start = 2)))/60))
newnam2
colnames(tma1) <- newnam2
...
pNPs <- resbind %>% 
  ...
  dplyr::filter(!grepl("stdcurve_2", variable)) %>% # stdcurve_1 
...
dat_normalized <- dat4 %>%
  dplyr::mutate(mean_1 = cur_data()[[5]]-cur_data()[[2]]) %>% # subtract the abiotic control
  dplyr::mutate(mean_2 = cur_data()[[6]]-cur_data()[[2]]) %>%
  dplyr::mutate(mean_3 = cur_data()[[7]]-cur_data()[[2]]) %>%
