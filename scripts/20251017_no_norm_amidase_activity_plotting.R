...
resmax <- resl %>%
  #dplyr::filter(r2 >= 0.5) %>% # make sure R^2 is above or equal to 0.9
  group_by(enz) %>%
...
pdf("output/split_by_substrate.pdf", width = 20)
pl
dev.off()
...
data <- read_csv("data/20251017/calculated_slopes.csv")
...
slopes_plot <- data_fc %>% # Use the data_fc dataframe with the new column
  ...
  scale_color_manual(
    ...
    labels = c("rfp" = "RFP Mean FC", "wt" = "WT Mean FC (Baseline 1.0)"),
    name = "Reference Line"
  ) +
  
  # E. Labels and Theme
  labs(
    title = "Distribution of Slopes by Substrate (Fold Change over WT)",
    x = "Reaction Slope (Fold Change over WT Mean)", # Updated x-axis label
    y = "Density"
  ) +
...
# Print the plot
pdf("data/20251017/histogram_fc_no_protein_norm.pdf", width = 12, height = 8, bg = "black")
print(slopes_plot)
invisible(dev.off())
