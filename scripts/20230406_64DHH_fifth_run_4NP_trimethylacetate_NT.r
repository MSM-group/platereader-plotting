...
enzym <- "20230406_64DHH_replication_4NP_trimethylacetate_5" # change this for p055 etc.
yyyymmdd <- "20230406"

# Read in the plate template
folder_path <- file.path(paste0("data/", yyyymmdd, "/"))
file_path <- list.files(folder_path, pattern = "Platelayout", full.names = T)
...
pNPs <- resbind %>% 
  ...
  #dplyr::filter(!grepl("stdcurve_1", variable)) %>%
