...
yyyymmdd <- "20230505"
pH_row <- "pH7_" # new entry, change for each pH
...
pNPs <- resbind %>% 
  ...
  dplyr::filter(!grepl("stdcurve_1", variable)) %>% # stdcurve_1 
...
dat4 <- dat2 %>%
  ...
  group_by(reps_combined, time) %>%
  summarise_each(funs(mean), nmols_pNP) %>%
  tidyr::spread(., reps_combined, nmols_pNP) # calculate mean activity
  
dat_normalized <- dat4 %>%
  dplyr::mutate(mean = cur_data()[[3]]-cur_data()[[2]]) %>%
  dplyr::mutate(variable = pH_row)

pdf(paste0("output/", yyyymmdd, "_", enzym,"_", pH_row, "_with_errorbars.pdf"), width = 14, height = 8)
pl <- ggplot(dat_normalized, aes(x=time, y=mean)) +
  geom_point() +
  labs(y = "nmol 4NP produced", x = "Time (minutes)") +
#  geom_errorbar(aes(ymax=mean + sd, ymin = mean - sd), width=0.3,size=0.6)+
  theme(legend.title=element_blank(), axis.line=element_line(color="black"),
...
slope_final
write_csv(slope_final, paste0("output/", yyyymmdd, "_", enzym, "_", pH_row, "_all_data_calculated_slopes.csv"))
